...
function frame(now) {
  const seconds = lastTime ? Math.min((now - lastTime) / 1000, 0.05) : 0;
  lastTime = now;
  if (state === 'playing') {
    ...
    const hitGround = birdBottom >= CONFIG.canvasHeight - CONFIG.groundHeight;
    const hitTop = birdTop <= 0;
